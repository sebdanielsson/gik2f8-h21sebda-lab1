const RenderResultsHTML = (searchResults) => {
    ...
    for (let i=0; i<searchResults.length; i++) {
         ...
         */
        resultsHTML += `
            <li onmouseenter="renderDetails(event)" onmouseleave="removeDetails(event)" id="book_${searchResults[i].id}" class="book-item flex flex-col p-4 rounded-lg dark:hover:bg-gray-600 hover:bg-gray-100">
                    <span class="text-xl sm:text-2xl">${searchResults[i].title}</span>
                    <span class="text-base"><a href="./?q=${searchResults[i].author}" class="text-purple-400">${searchResults[i].author}</a></span>
                </div>
            </li>
        `;
    }
